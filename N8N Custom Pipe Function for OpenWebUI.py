...
author: Rodrigo Garcia Saura (fork from https://openwebui.com/f/coleam/n8n_pipe)
"""
...
class Pipe:
    ...
    def _extract_user_metadata(self, user: Optional[Dict[str, Any]]) -> Dict[str, str]:
        """Extract user metadata for the payload.

        Args:
            user: User information dictionary

        Returns:
            Dictionary containing user metadata fields
        """
        if not user:
            return {
                "user_id": "anonymous",
    ...
    def _extract_chat_id(self, body: Dict[str, Any]) -> str:
        """Extract or generate chat ID from the request body.

        Args:
            body: Request body containing chat information

        Returns:
            Chat ID string
        """
        # Try to extract chat_id from body, or generate one if not present
        return body.get("chat_id", body.get("id", str(uuid.uuid4())))
    ...
    async def pipe(
        ...
        try:
            ...
            user_metadata = self._extract_user_metadata(__user__)

            # Generate message and chat IDs
            ...
            payload = {
                ...
                "user_email": user_metadata["user_email"],
